...
def get_dimension_table_cols_dict(sql_dict):
    dim_table_cols = {}

    for table in sql_dict['tables']:
        table_name = table['name']
        dim_table_cols[table_name] = table['columns']
    return dim_table_cols
...
def prepare_create_tables_sql():
    ...
    dim_table_cols = get_dimension_table_cols_dict(sql_dict)

    INDENT = '	'

    with open('../sql/create_procedures.sql', 'w') as sql_file:
        # create procedures
        for fact_table in sql_dict['tables']:
        ...
        for fact_table in sql_dict['fact_tables']:
            name = fact_table['name']
            fact_columns = fact_table['columns']
